...
use super::format::{
    ...
    V6_SECRET_ALGORITHM, V6_SUITE, V6_VAULT_VERSION, V6_X25519_ALGORITHM, VAULT_VERSION, Vault,
    X25519KeyPair,
};
use crate::crypto::{
...
pub fn unlock_vault(passphrase: &str, vault_path: &str) -> Result<UnlockedVault> {
    ...
    match probe.version {
        ...
        version if version < VAULT_VERSION => {
            ...
            unlock_v6(vault, passphrase, vault_path)
        }
        V6_VAULT_VERSION => {
...
fn verify_v6_key_commitment(vault: &Vault, master_key: &MasterKey) -> Result<()> {
...
mod tests {
    ...
    fn stage_vault_as_v6(vault: &mut Vault, passphrase: &str) {
        vault.version = V6_VAULT_VERSION;
        vault.min_version = V6_VAULT_VERSION;
        vault.kem.algorithm = V6_KEM_ALGORITHM.to_string();
        vault.x25519.algorithm = V6_X25519_ALGORITHM.to_string();
        vault.suite = V6_SUITE.to_string();
        for secret in vault.secrets.values_mut() {
            secret.algorithm = V6_SECRET_ALGORITHM.to_string();
        }
        ...
        vault.key_commitment = Some(compute_v6_key_commitment(&master_key, vault).unwrap());
    }

    fn write_staged_v6_vault(vault_path: &str, passphrase: &str) -> Vault {
        create_vault(passphrase, vault_path).unwrap();
        let mut vault: Vault =
            serde_json::from_str(&fs::read_to_string(vault_path).unwrap()).unwrap();
        stage_vault_as_v6(&mut vault, passphrase);
    ...
    fn test_unlock_staged_v6_vault() {
        ...
        write_staged_v6_vault(vault_path, "test-passphrase");
        let unlocked = unlock_vault("test-passphrase", vault_path).unwrap();

        assert_eq!(unlocked.vault.version, V6_VAULT_VERSION);
        assert_eq!(unlocked.vault.min_version, V6_VAULT_VERSION);
        assert_eq!(unlocked.vault.x25519.algorithm, V6_X25519_ALGORITHM);
        assert_eq!(unlocked.vault.suite, V6_SUITE);
    }
    ...
    fn test_unlock_v6_rejects_future_min_version() {
        ...
        let mut vault = write_staged_v6_vault(vault_path, "test-passphrase");
        vault.min_version = V6_VAULT_VERSION + 1;
        let kdf_config = KdfConfig {
            salt: vault.kdf.salt.clone(),
            time_cost: vault.kdf.time_cost,
            memory_cost: vault.kdf.memory_cost,
    ...
    fn test_set_and_get_secret_on_staged_v6_vault_uses_v6_algorithm() {
        ...
        write_staged_v6_vault(vault_path, "test-pass");
        let mut unlocked = unlock_vault("test-pass", vault_path).unwrap();
    ...
    fn test_unlock_v6_checks_commitment_before_private_key_decrypt() {
        ...
        let mut vault = write_staged_v6_vault(vault_path, "test-pass");
    ...
    fn test_change_passphrase_preserves_staged_v6_metadata() {
        ...
        write_staged_v6_vault(vault_path, "old-pass");
        let mut unlocked = unlock_vault("old-pass", vault_path).unwrap();
        change_passphrase(&mut unlocked, "new-pass").unwrap();

        let updated: Vault =
            serde_json::from_str(&fs::read_to_string(vault_path).unwrap()).unwrap();
        assert_eq!(updated.version, V6_VAULT_VERSION);
        assert_eq!(updated.min_version, V6_VAULT_VERSION);
        assert_eq!(updated.x25519.algorithm, V6_X25519_ALGORITHM);
        assert_eq!(updated.suite, V6_SUITE);

        let reopened = unlock_vault("new-pass", vault_path).unwrap();
        assert_eq!(reopened.vault.version, V6_VAULT_VERSION);
    }
